...
if __name__ == '__main__':
    ...
    places = dict()
    constitutions = dict()
    with open("ruc_news_out.txt", "w", encoding="utf-8-sig") as f_out:
        with open("ruc_news_out_keywords.txt", "w", encoding="utf-8-sig") as f_out_:
            pass
    with open("homework_input_ruc_news.txt", "r", encoding="utf-8-sig") as f_in:
        ...
        while line != "":
            # 分词，写入
            with open("ruc_news_out.txt", "a", encoding="utf-8-sig") as f_out:
                linels = line.split("\t")
            ...
            for word in allword:
                if word in allwords:
                    ...
                    allwords[word] = 1
            # 抽取关键词
            with open("ruc_news_out_keywords.txt", "a", encoding="utf-8-sig") as f_out:
    ...
    with open("ruc_news_out_keywords_stat.csv", "w", encoding="utf-8-sig") as f_out:
        f_out.write("全部词语：\n")
        txt = ""
        allwordsls = []
        ...
        while True:
            ...
            i += 1
        f_out.write("\n")
        w = wordcloud.WordCloud(font_path="msyh.ttc", width=500, height=500, background_color="white")
        w.generate(txt)
        w.to_file("全部词语.png")
        ...
        while True:
            elem = namesls[i]
            if elem[1] not in ["师生", "养老", "智慧", "立德", "明德", "王", "高水平", "从严治党", "大力支持", "文明",
                               "史", "荣誉", "荣获", "盖章", "卓越", "关怀", "祝贺", "老同志", "宝钢"]:
                f_out.write(elem[1] + "," + str(elem[0]) + "\n")
                txt += (elem[1] + " ") * elem[0]
                count += 1
